...
create_flu_data_targets <- function() {
  skip_veteran <- tolower(Sys.getenv("SKIP_VETERAN_DATA", "")) %in% c("1", "true", "yes")
  ...
  rlang::list2(
    ...
    tar_target(
      ...
      command = {
        if (skip_veteran) {
          message("SKIP_VETERAN_DATA=1 detectado: omitiendo veteran_state_archive (flu).")
          return(NULL)
        }
    ...
    tar_target(
      ...
      command = {
        ...
        if (!skip_veteran) {
          joined_archive_data <- joined_archive_data %>% epix_merge(veteran_state_archive, sync = "locf")
        } else {
          message("SKIP_VETERAN_DATA=1 detectado: omitiendo merge con veteran_state_archive (flu).")
        }
        joined_archive_data %<>%
          extract2("DT") %>%
          drop_na("hhs") %>%
